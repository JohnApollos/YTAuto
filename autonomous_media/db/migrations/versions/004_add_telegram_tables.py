...
def upgrade() -> None:
    ...
    # telegram_configs
    if 'telegram_configs' not in existing_tables:
        op.create_table(
            'telegram_configs',
    ...
    if 'telegram_delivery_logs' not in existing_tables:
        op.create_table(
            'telegram_delivery_logs',
            sa.Column('id', postgresql.UUID(as_uuid=True), primary_key=True),
...
def downgrade() -> None:
    ...
    if 'telegram_delivery_logs' in existing_tables:
        op.drop_table('telegram_delivery_logs')
    if 'telegram_configs' in existing_tables:
